...
get '/test1' do
  '<html>
<body>
<img src="http://localhost:4567/res/img/9" exif="true" id="MyPrettyImage">
...
get '/test' do
	...
	res = ""
	obj = EXIFR::JPEG.new(File.dirname(__FILE__)+'/static/exif_test_files/05_01_Teckla_avatar.jpg')
...
#  params[:captures][0]
			...
			#r = Resource.new
			#p File.basename("http://ya.ru/")
			#File.open("log.txt", "w"){|f| f.write(File.dirname ENV["input"])}
			#puts File.size ENV["input"]
			#Find.find('./') do |f| p f if File.size? f end
			#p Dir['D:/pics/*.*']
			#Find.find(ENV["home"]+'/')do |f| p f end